...
model {
  ...
  // priors
  cop_mat ~ lkj_corr(2);
  Log_scale0 ~ normal(0,10);
  // Log_scale1 ~ normal(0,100);
  // Log_scale2 ~ normal(0,100);
  shape ~ normal(0,1);
  alpha_loc0 ~ normal(0,10);
  alpha_loc1 ~ normal(0,10);
  // alpha_loc2 ~ normal(0,100);
  for (r in 1:S){
    // print("station ",r);
    tmp2 = gev_v_log_v_1cov_SgST(col(y,r), covar1[,r], alpha_loc0[r], alpha_loc1[r], Log_scale0[r], shape[r]);
    tmp3 = gev_v_cdf_v_1cov_SgST(col(y,r), covar1[,r], alpha_loc0[r], alpha_loc1[r], Log_scale0[r], shape[r]);
    for (i in 1:N){
      lp[i,r] = tmp2[i];
      Fs[i,r] = tmp3[i]; 
    }
